...
const EmailModal: React.FC<EmailModalProps> = ({ isOpen, onClose, membershipType }) => {
  ...
  const handleEmailSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // 1. Primeiro salvamos o email (seu código existente)
      ...
      if (!paymentResponse.ok) throw new Error('Falha ao criar pagamento');
...
// И ОБНОВИТЕ ЭТУ СТРОКУ в основном компоненте:
